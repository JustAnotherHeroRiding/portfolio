...
const ProductBase = ({ accent }: { accent: string }) => (
  <group position={[0, -0.9, 0]}>
    <RoundedBox args={[2.45, 0.2, 1.8]} radius={0.14} smoothness={5} castShadow receiveShadow>
      <meshStandardMaterial color={colors.graphite} roughness={0.42} metalness={0.12} />
    </RoundedBox>
    <RoundedBox args={[0.42, 0.07, 0.04]} radius={0.035} smoothness={4} position={[0, 0.01, 0.92]}>
  ...
  return (
    <group>
      ...
      <group position={[0, 0.2, 0]} rotation={[0.03, -0.08, 0]}>
        ...
        <RoundedBox args={[0.98, 1.55, 0.04]} radius={0.13} smoothness={5} position={[0, 0, 0.12]}>
          <meshStandardMaterial color={colors.aluminum} roughness={0.5} />
        </RoundedBox>
      ...
      <group position={[0, -0.55, 0.52]} rotation={[-0.3, 0, 0]}>
        {[0, 1, 2].map(page => (
          <RoundedBox
            ...
            radius={0.06}
            smoothness={4}
            position={[(page - 1) * 0.035, page * 0.055, 0]}
            rotation={[0, (page - 1) * 0.02, 0]}
            castShadow
          >
            <meshStandardMaterial color={colors.white} roughness={0.68} />
          </RoundedBox>
        ))}
        {[0.18, -0.04, -0.26].map((z, index) => (
          <RoundedBox key={z} args={[0.82 - index * 0.1, 0.018, 0.035]} radius={0.01} position={[-0.18, 0.18, z]}>
    ...
    <group>
      ...
      <group position={[0, -0.05, 0]}>
        <mesh position={[0, -0.35, 0]} castShadow>
          <cylinderGeometry args={[0.12, 0.16, 1.1, 24]} />
          <meshStandardMaterial color={colors.aluminum} roughness={0.25} metalness={0.65} />
        </mesh>
        <RoundedBox args={[0.95, 0.2, 0.28]} radius={0.1} smoothness={5} position={[0, 0.18, 0]} castShadow>
          <meshStandardMaterial color={colors.aluminum} roughness={0.25} metalness={0.65} />
        </RoundedBox>
        {[-0.37, 0.37].map(x => (
          <group key={x}>
            <mesh position={[x, 0.82, 0]} castShadow>
              <cylinderGeometry args={[0.105, 0.105, 1.35, 24]} />
              <meshStandardMaterial color={colors.aluminum} roughness={0.25} metalness={0.65} />
            </mesh>
            <mesh position={[x, 1.5, 0]}>
              <sphereGeometry args={[0.105, 20, 16]} />
              <meshStandardMaterial color={colors.cyan} emissive={colors.cyan} emissiveIntensity={0.22} />
            </mesh>
          </group>
        ))}
      </group>
      <group ref={waves} position={[0, -0.02, -0.03]} rotation={[Math.PI / 2, 0, 0]}>
        {[1.05, 1.32].map(radius => (
          <mesh key={radius}>
            <torusGeometry args={[radius, 0.035, 10, 64]} />
            <meshStandardMaterial color={colors.purple} emissive={colors.purple} emissiveIntensity={0.25} />
          </mesh>
        ))}
      </group>
    </group>
...
const WiseMarginArtifact = () => (
  <group>
    ...
    <RoundedBox args={[2, 1.45, 0.28]} radius={0.2} smoothness={6} position={[0, 0.25, 0]} castShadow>
      <meshStandardMaterial color={colors.white} roughness={0.42} />
    </RoundedBox>
    <RoundedBox args={[1.68, 1.12, 0.04]} radius={0.13} smoothness={5} position={[0, 0.25, 0.17]}>
    ...
    <mesh position={[0.75, -0.63, 0.2]}>
      ...
      <meshStandardMaterial color={colors.green} emissive={colors.green} emissiveIntensity={0.35} />
    </mesh>
  </group>
)
...
const MATechnixArtifact = () => (
  <group>
    <ProductBase accent={colors.green} />
    <RoundedBox args={[1.85, 1.15, 1.25]} radius={0.16} smoothness={5} position={[0, -0.1, 0]} castShadow>
      <meshStandardMaterial color={colors.graphite} roughness={0.48} />
    </RoundedBox>
    <RoundedBox args={[1.65, 0.16, 1.05]} radius={0.08} smoothness={4} position={[0, 0.56, -0.08]} rotation={[-0.18, 0, 0]} castShadow>
      <meshStandardMaterial color={colors.aluminum} roughness={0.42} />
    </RoundedBox>
    {[
      ...
      <Float key={index} speed={1 + index * 0.15} floatIntensity={0.12} rotationIntensity={0.04}>
        <RoundedBox args={[0.42, 0.42, 0.42]} radius={0.08} smoothness={4} position={[x as number, y as number, z as number]} castShadow>
          <meshStandardMaterial color={color as string} roughness={0.38} />
        </RoundedBox>
      </Float>
    ))}
    {[0, 1, 2].map(index => (
...
const WhatTheKeyArtifact = () => {
  ...
  return (
    <group>
      ...
      <RoundedBox args={[2.2, 0.38, 1.35]} radius={0.16} smoothness={5} position={[0, -0.12, 0]} castShadow>
        <meshStandardMaterial color={colors.graphite} roughness={0.34} />
      </RoundedBox>
      <group ref={keys} position={[0, 0.11, 0.05]}>
        {Array.from({ length: 7 }, (_, index) => (
          <RoundedBox key={index} args={[0.27, 0.14, 1.04]} radius={0.045} smoothness={4} position={[(index - 3) * 0.29, 0, 0]} castShadow>
            <meshStandardMaterial color={colors.white} roughness={0.42} />
          </RoundedBox>
        ))}
      </group>
      {[0, 1, 3, 4, 5].map(index => (
        <RoundedBox key={index} args={[0.16, 0.2, 0.62]} radius={0.04} smoothness={4} position={[(index - 2.5) * 0.29, 0.28, -0.22]} castShadow>
...
const CherryPickArtifact = () => (
  <group>
    ...
    <RoundedBox args={[2.15, 1.65, 0.18]} radius={0.2} smoothness={6} position={[0, 0.18, -0.1]} castShadow>
      <meshStandardMaterial color={colors.white} roughness={0.48} />
    </RoundedBox>
    <group position={[0, 0.2, 0.03]}>
      ...
      {[
        ...
        <mesh key={index} position={[x as number, y as number, 0.05]}>
          <sphereGeometry args={[color === colors.red ? 0.15 : 0.105, 20, 16]} />
          <meshStandardMaterial color={color as string} roughness={0.38} />
        </mesh>
      ))}
    </group>
  ...
  return (
    <group>
      ...
      <group position={[0, -0.2, 0]}>
        <RoundedBox args={[2.05, 0.16, 1.3]} radius={0.11} smoothness={5} rotation={[-0.08, 0, 0]} castShadow>
          <meshStandardMaterial color={colors.aluminum} roughness={0.28} metalness={0.5} />
        </RoundedBox>
        <RoundedBox args={[0.5, 0.02, 0.32]} radius={0.03} position={[0, 0.1, 0.12]}>
          <meshStandardMaterial color={colors.dark} roughness={0.42} />
        </RoundedBox>
      </group>
      <group ref={screen} position={[0, -0.05, -0.55]} rotation={[-0.18, 0, 0]}>
        <RoundedBox args={[2, 1.35, 0.13]} radius={0.13} smoothness={5} position={[0, 0.67, 0]} castShadow>
          <meshStandardMaterial color={colors.aluminum} roughness={0.28} metalness={0.5} />
        </RoundedBox>
        <RoundedBox args={[1.75, 1.1, 0.035]} radius={0.08} position={[0, 0.67, 0.08]}>
          <meshStandardMaterial color={colors.graphite} roughness={0.34} />
        </RoundedBox>
        <mesh position={[0.52, 0.72, 0.11]} rotation={[0, 0, Math.PI * 0.18]}>
          <torusGeometry args={[0.25, 0.055, 10, 36, Math.PI * 1.45]} />
          <meshStandardMaterial color={colors.cyan} emissive={colors.cyan} emissiveIntensity={0.48} />
        </mesh>
      </group>
      <mesh position={[0.82, -0.11, 0.62]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.045, 0.045, 0.025, 20]} />
        <meshStandardMaterial color={colors.cyan} emissive={colors.cyan} emissiveIntensity={0.65} />
      </mesh>
    </group>
  )
}
...
export const ArtifactSlot = ({ name, index, count, activeIndex, dragOffset, onSelect }: ArtifactSlotProps) => {
  ...
  useFrame((_, delta) => {
    ...
    group.current.position.z = MathUtils.damp(group.current.position.z, -Math.abs(Math.sin(angle)) * 2.3, 7, delta)
